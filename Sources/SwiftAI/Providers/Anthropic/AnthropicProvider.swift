...
public actor AnthropicProvider: AIProvider, TextGenerator {
    ...
    ///
    /// This method returns an asynchronous stream that emits text fragments
    /// as they are generated, allowing for real-time display of output.
    ///
    /// ## Usage
    /// ```swift
    /// let provider = AnthropicProvider(apiKey: "sk-ant-...")
    ...
    /// - Canceling the task stops generation
    ///
    /// ## Implementation Status
    ///
    /// - Note: This method will be fully implemented in Phase 5.
    ///   Currently returns a fatal error.
    ///
    /// - Parameters:
    ///   - prompt: The input text to generate a response for.
    ///   - model: The Claude model to use (e.g., `.claudeOpus45`).
    ...
    nonisolated public func stream(
        ...
        fatalError("Implementation in Phase 5")
    }

    /// Streams text generation with full chunk metadata.
    ///
    /// This method provides the most detailed streaming interface, emitting
    /// `GenerationChunk` objects that include not only the text but also
    /// metadata such as finish reasons, token usage, and timing information.
    ///
    /// ## Usage
    /// ```swift
    /// let provider = AnthropicProvider(apiKey: "sk-ant-...")
    ...
    /// - `text`: Text fragment (may be nil for metadata-only chunks)
    /// - `finishReason`: Present in final chunk, indicates why generation stopped
    /// - `usage`: Token usage statistics (typically in final chunk)
    ...
    /// Streams generation chunks for the given messages.
    ///
    /// This is the core streaming method required by the `AIProvider` protocol.
    /// It returns an asynchronous stream that yields `GenerationChunk` objects
