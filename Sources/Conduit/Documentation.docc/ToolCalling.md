...
During streaming, tool calls arrive progressively as ``PartialToolCall`` values. See <doc:Streaming> for details on handling `partialToolCall` and `completedToolCalls` in ``GenerationChunk``.

## ChatSession with Tools

``ChatSession`` handles the tool loop automatically. See <doc:ChatSession> for details:
